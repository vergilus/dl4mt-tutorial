...
class BleuValidator:
    ...
    def build_bleu_cmd(self, trans_file):
        return "perl %(script)s %(answer)s < %(transFile)s" \
                %{'script':self.valid_path+self.bleu_script,
                  'answer':self.valid_trg,
    ...
    def decode(self, device, trans_saveto, model_file):
        ...
        '''
        cmd = "THEANO_FLAGS='device=%(device)s' python %(script)s -n -messageOff -k 5 %(model)s %(srcDict)s %(trgDict)s %(src)s %(trg)s" \
            %{'device':device, 
              ...
              'src':self.valid_src,
              'trg':os.path.join(self.temp_dir,trans_saveto)
              }
        print 'running: %s' %cmd
    ...
    def testBLEU(self, trans_file):
        ...
        if bleu==-1:
            print 'Fail to run script:%s for file:%s'\
            % (self.bleu_script,trans_file)
            sys.exit(0)
        return bleu
    ...
    def remove_temp_file(self, model_file,trans_file):
        ...
        '''
        cmd = 'rm %(transFile)s  %(model)s'%{'transFile':self.temp_dir+trans_file,
                          'model':model_file}
        #print cmd
        os.system(cmd)
        return
